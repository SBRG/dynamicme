...
class BendersRadixEstimator(RadixEstimator):
    """
    Radix-based discretization estimator.
    Uses Benders decomposition.
    """
    def __init__(self, *args, **kwargs):
        self.master = None
        self.sub    = None
        super(BendersRadixEstimator, self).__init__(*args,**kwargs)
    ...
    def fit(self, base_model, df_X, df_Y, **kwargs):
        ...
        #----------------------------------------------------
        # Instead of stacking into a big problem, split into
        # master and subproblems
        # stacker.stack_models(base_model, df_X, col_ind)
        #----------------------------------------------------
        # Construct the distributed radix problem in the correct sequence
        # 1) make condition-specific model
        # 2) add strong duality constraint
        # 3) add radix formulation
        #   new vars: y_klj (common across submodels)
        #             z_rklj (subprob specific, linked to y_klj)
        #             z_L, z_U, zdiff_L, zdiff_U (subprob specific, linked to y_klj) 
        #             cons_digs: only in master
        #             force_nonzero: only in master
        # 4) add error minimization objective if desired
        # 5) extract A, B, cx, x, needed to make subproblem and share y across all problems.
        #----------------------------------------------------
        # Create base submodel--make condition-specific submodels using it
        opt = Optimizer(base_model)
        mdl = opt.add_duality_gap_constraint(INF=self._INF, inplace=True)
        cons_ref = base_model.metabolites.get_by_id(fit_constraint_id)
        var_cons_dict = {}
        for rxn_ref in cons_ref.reactions:
            constraint_p = mdl.metabolites.get_by_id(fit_constraint_id)
            var_d = mdl.reactions.get_by_id('wa_%s'%constraint_p.id)
            rxn_p = mdl.reactions.get_by_id(rxn_ref.id)
            # Get coeff in dual
            cons_ds = [m for m in var_d.metabolites.keys() if rxn_p.id==m.id]
            a0 = rxn_p.metabolites[constraint_p]
            if var_cons_dict.has_key(rxn_ref.id):
        ...
        prevent_zero = self.prevent_zero
        opt.to_radix(mdl, var_cons_dict,
                radix,powers,digits=digits,prevent_zero=prevent_zero)

        if max_nonzero_binaries is not None:
            self.limit_nonzero_binaries(mdl, max_nonzero_binaries)
        # Create master using this base model
        master = BendersMaster(mdl)
        self.master = master

        #for mdl_ind, mdl in splitter.yield_models(base_model, df_X, col_ind):
        # Add objective function for specific condition
        conds = df_X[col_ind].unique()
        for cind,cond in enumerate(conds):
            dfi = df_X[df_X[col_ind]==cond]
            suffix = '_%s'%cond
            mdli = copy_model(mdl, suffix=suffix)
            for i,row in dfi.iterrows():
                rxn = mdli.reactions.get_by_id(row['rxn']+suffix)
                rxn.lower_bound = row['lb']
                rxn.upper_bound = row['ub']
                rxn.objective_coefficient = row['obj']  # affects duality gap cons

        df_Yi = df_Y[ df_Y[col_ind]==mdl_ind]
        self.make_objective(master, gap, var_cons_dict, mdl_ind,
                df_Yi, col_meas_id, col_meas_val, col_ind,
                objective, reg_weight)
        sub = BendersSubmodel(gap, 'base') #, mdl_ind)
        self.submodel = sub
    ...
    def make_objective(self, master, mdl, var_cons_dict, mdl_ind,
            df_meas, col_meas_id, col_meas_val, col_ind,
        ...
        if objective == 'minerr':
            if reset_obj:
                for rxn in mdl.reactions:
                    rxn.objective_coefficient = 0.
            for rind,row in df_meas.iterrows():
                x_meas = row[col_meas_val]
                meas_id = row[col_meas_id]
                rxn_meas = mdl.reactions.get_by_id(meas_id+"_%s"%mdl_ind)
            ...
            for group_id in var_cons_dict.keys():
                for l,pwr in enumerate(powers):
                    for k,digit in enumerate(digits):
                        yid = 'binary_%s%s%s'%(group_id,k,l)
                        y   = master.cobra_model.reactions.get_by_id(yid)
                        # Prefer pwr=0, digit=1
                        if pwr==0 and digit==1:
                            y.objective_coefficient=0.
                        else:
